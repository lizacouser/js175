...
const validateName = (name, whichName) => {
  return body(name)
    ...
    .bail()
    .isLength({max: 25})
    .withMessage(`${whichName} too long. Maximum length is 25 characters.`)
    .isAlpha()
    .withMessage(`${whichName} contains invalid characters.`)
}
...
app.post("/contacts/new",
  ...
  // middleware reloads page and displays error messages
  (req, res, next) => {
    ...
    if (!errors.isEmpty()) {
      ...
      res.render("addContact", {
        flash: req.flash(), // returns flash object
  ...
  // middleware add contact if no errors
  (req, res) => {
